...
def hashlib_md5sum(name, package, definition):
    ...
    for line in definition:
        ...
        if value != None:
            ...
            str = str + "=" + value
    hl = hashlib.md5()
    hl.update(str.encode())  # 修正：將字串轉為 bytes
    return hl.hexdigest()
...
def messages_generate(path, build, msgs):
    ...
    for d in sorted(os.listdir(mydir)):
	    try:
	        ...
	        print(traceback.format_exc())  # 顯示完整 traceback
...
def roslib_copy_examples_files(path):
    ...
    f.write('  src/examples/publisher/ExamplePublisher.cpp\n')
    f.write('  ${TINYROS_SRC_LIST}\n')
    f.write(')\n\n')
    f.write('add_executable(ExampleSubscriber\n')
    f.write('  src/examples/subscriber/ExampleSubscriber.cpp\n')
    f.write('  ${TINYROS_SRC_LIST}\n')
    f.write(')\n\n')
    f.write('add_executable(ExampleService\n')
    f.write('  src/examples/service/ExampleService.cpp\n')
    f.write('  ${TINYROS_SRC_LIST}\n')
    f.write(')\n\n')
    f.write('add_executable(ExampleServiceClient\n')
    f.write('  src/examples/service_client/ExampleServiceClient.cpp\n')
    f.write('  ${TINYROS_SRC_LIST}\n')
    f.write(')\n\n')
    f.write('IF (CMAKE_SYSTEM_NAME MATCHES "CYGWIN")\n')
    f.write('  target_link_libraries(ExamplePublisher libpthread.a)\n')
    f.write('  target_link_libraries(ExampleSubscriber libpthread.a)\n')
    f.write('  target_link_libraries(ExampleService libpthread.a)\n')
    f.write('  target_link_libraries(ExampleServiceClient libpthread.a)\n')
    f.write('ENDIF()\n\n')
    f.write('IF (CMAKE_SYSTEM_NAME MATCHES "Linux")\n')
    f.write('  target_link_libraries(ExamplePublisher libpthread.so)\n')
    f.write('  target_link_libraries(ExampleSubscriber libpthread.so)\n')
    f.write('  target_link_libraries(ExampleService libpthread.so)\n')
    f.write('  target_link_libraries(ExampleServiceClient libpthread.so)\n')
    f.write('ENDIF()\n\n')
    f.close()
